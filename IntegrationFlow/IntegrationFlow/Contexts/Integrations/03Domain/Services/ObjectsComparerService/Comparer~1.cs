...
using System.Text;
using IntegrationFlow.Contexts.Integrations._03Domain.Services.ObjectsComparerService.CustomComparers;
...
namespace IntegrationFlow.Contexts.Integrations._03Domain.Services.ObjectsComparerService
{
    ...
    public class Comparer<T> : AbstractComparer<T>
    {
        ...
        private List<PropertyInfo> GetProperties(Type type, List<Type> processedTypes)
        {
            ...
            if (type.IsInterface)
            {
                foreach (var parrentInterface in type.GetInterfaces())
                {
